...
function ExperimentChat() {
  ...
  useEffect(() => {
    console.log('🔄 ExperimentChat 컴포넌트 마운트됨');
    
    // 새로고침 시 채팅 기록 복원 (인증 확인보다 먼저)
    ...
    console.log('💾 저장된 실험 정보:', savedExperimentDetails);
    
    if (savedMessages && savedMessages !== 'undefined') {
      try {
        const parsedMessages = JSON.parse(savedMessages);
        console.log('✅ 채팅 기록 복원 성공:', parsedMessages);
        setMessages(parsedMessages);
      } catch (error) {
        console.error('❌ 채팅 기록 복원 실패:', error);
      }
    } else {
      console.log('ℹ️ 저장된 채팅 기록이 없습니다.');
    }
    
    if (savedExperimentDetails && savedExperimentDetails !== 'undefined') {
      try {
        const parsedDetails = JSON.parse(savedExperimentDetails);
        console.log('✅ 실험 정보 복원 성공:', parsedDetails);
        setExperimentDetails(parsedDetails);
      } catch (error) {
        console.error('❌ 실험 정보 복원 실패:', error);
      }
    } else {
      console.log('ℹ️ 저장된 실험 정보가 없습니다.');
    }
  ...
  useEffect(() => {
    ...
    if (messages.length > 0) {
      sessionStorage.setItem('experimentChatMessages', JSON.stringify(messages));
    }
  }, [messages]);
  
  useEffect(() => {
    // 실험 정보 변경 시 sessionStorage에 저장
