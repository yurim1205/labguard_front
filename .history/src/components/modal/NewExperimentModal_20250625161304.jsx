...
const NewExperimentModal = ({ onClose, onTitleSubmit }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 임시로 API 호출 없이 바로 채팅 페이지로 이동
      const selectedManualData = manuals.find(
        (manual) => manual.manual_id === parseInt(selectedManual)
      );
      
      // 임시 experiment_id 생성 (실제로는 백엔드에서 받아야 함)
      const experiment_id = Date.now().toString();
      ...
      navigate(`/ExperimentChat/${experiment_id}`, {
        state: {
          experiment_id,
          experiment_title,
          manual: selectedManualData?.manual_id || selectedManual,
        },
      });
