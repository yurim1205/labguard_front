...
function ExperimentChat() {
  ...
  useEffect(() => {
    ...
    if (savedExperimentDetails) {
      try {
        ...
        // location.state의 데이터를 우선적으로 사용
        const finalDetails = {
          experiment_title: location.state?.experiment_title || parsedDetails.experiment_title || '실험 제목 없음',
          manual: location.state?.manual || parsedDetails.manual || null,
        };
        setExperimentDetails(finalDetails);
        console.log('새로고침 후 실험 정보 복원 (location.state 우선):', finalDetails);
      } catch (error) {
  ...
  // experimentDetails 변경 시 sessionStorage에 저장
  useEffect(() => {
    if (experimentDetails.experiment_title) {
      sessionStorage.setItem('experimentDetails', JSON.stringify(experimentDetails));
      console.log('실험 정보 sessionStorage 저장:', experimentDetails);
    }
  }, [experimentDetails]);
