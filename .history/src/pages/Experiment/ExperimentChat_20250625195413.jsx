...
function ExperimentChat() {
  ...
  useEffect(() => {
    ...
    // 새로고침 시 채팅 기록 복원
    const savedMessages = sessionStorage.getItem('experimentChatMessages');
    const savedExperimentDetails = sessionStorage.getItem('experimentDetails');
    
    if (savedMessages) {
      try {
        const parsedMessages = JSON.parse(savedMessages);
        setMessages(parsedMessages);
        console.log('새로고침 후 채팅 기록 복원:', parsedMessages);
      } catch (error) {
        console.error('채팅 기록 복원 실패:', error);
      }
    }
    
    if (savedExperimentDetails) {
      try {
        const parsedDetails = JSON.parse(savedExperimentDetails);
        setExperimentDetails(parsedDetails);
        console.log('새로고침 후 실험 정보 복원:', parsedDetails);
      } catch (error) {
        console.error('실험 정보 복원 실패:', error);
      }
    }
      ...
      try {
        ...
        if (res.ok) {
          ...
          setExperimentDetails({
            experiment_title: experimentData.title || experimentData.experiment_title || '실험 제목 없음',
            manual: experimentData.manual_id || experimentData.manual || null,
          });
          ...
          console.warn("세션을 찾을 수 없습니다. 실험 ID로 정보를 조회합니다.");
          
          // 세션이 없으면 실험 ID로 정보 조회 시도
          const experimentId = location.state?.experiment_id;
          if (experimentId) {
            try {
              const expRes = await fetch(`/api/experiment/${experimentId}`, {
                method: 'GET',
                credentials: 'include'
              });
              ...
              if (expRes.ok) {
                ...
                console.log("실험 ID로 정보 로드 성공:", expData);
                
                setExperimentDetails({
                  experiment_title: expData.title || expData.experiment_title || '실험 제목 없음',
                  manual: expData.manual_id || expData.manual || null,
                });
                
                setMessages([
                  { 
                    sender: 'bot', 
                    text: `안녕하세요! "${expData.title || expData.experiment_title || '실험'}" 실험에 오신 것을 환영합니다! 🧑‍🔬\n\n실험에 대해 궁금한 점이 있으시면 언제든 질문해주세요!`
                  }
                ]);
              } else {
                // 실험 ID로도 찾을 수 없으면 기본 메시지
                setMessages([
                  { 
                    sender: 'bot', 
              ...
              console.error("실험 ID로 정보 조회 실패:", error);
              setMessages([
                { 
                  sender: 'bot', 
                  text: `안녕하세요! "${experimentDetails.experiment_title}" 실험에 오신 것을 환영합니다! 🧑‍🔬\n\n${experimentDetails.manual ? `선택하신 매뉴얼: ${experimentDetails.manual.filename || experimentDetails.manual.title || '매뉴얼'}` : '매뉴얼이 선택되지 않았습니다.'}\n\n실험에 대해 궁금한 점이 있으시면 언제든 질문해주세요!`
