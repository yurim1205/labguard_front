        ...
        <section className="mt-[48px]">
          ...
          <ul className="bg-white border border-[#b5b5b5] rounded-[10px] py-4 list-none m-0 mt-[1px]">
            {manuals.map((m, i) => (
              <li
                key={i}
                className="flex items-center justify-between px-8 py-5 border-b last:border-b-0 text-[1.05rem]"
              >
                <div className="flex items-center gap-4">
                  <img src={equipment} alt="equipment" className="w-[28px] h-[28px]" />
                  <span className=" text-[#33308B] font-semibold underline">{m.name}</span>
                </div>
                <span className="mt-[12px] mb-[12px] text-[#33308B] text-[0.95rem]">{m.date}</span>
              </li>
            ))}
          </ul>
        </section>
