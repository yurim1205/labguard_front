...
function ExperimentChat() {
  ...
  const [statusText, setStatusText] = useState('텍스트 또는 음성 모드를 선택해주세요');
  ...
  const connectWebSocket = () => {
    ...
    socketRef.current.onopen = () => {
      ...
      setStatusText('텍스트 모드 활성화 - WebSocket 연결 완료');
    ...
    socketRef.current.onerror = (error) => {
      ...
      setStatusText('텍스트 모드 활성화 - WebSocket 실패, HTTP API 사용');
    };
    ...
    socketRef.current.onclose = (event) => {
      ...
      if (event.code === 1008 || event.code === 1011) {
        ...
        console.log('WebSocket 연결 실패 (1006) - HTTP API로 대체');
        setStatusText('텍스트 모드 활성화 - WebSocket 실패, HTTP API 사용');
      } else {
        setStatusText('텍스트 모드 활성화 - WebSocket 연결 종료, HTTP API 사용');
      }
    };
  };
