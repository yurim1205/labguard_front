...
function ExperimentChat() {
  ...
  useEffect(() => {
    ...
    // location.state가 있으면 우선 사용, 없으면 sessionStorage 사용
    if (location.state?.experiment_title) {
      ...
      console.log('location.state에서 실험 정보 설정:', locationDetails);
    } else if (savedExperimentDetails) {
      try {
        const parsedDetails = JSON.parse(savedExperimentDetails);
        setExperimentDetails(parsedDetails);
        console.log('sessionStorage에서 실험 정보 복원:', parsedDetails);
      } catch (error) {
