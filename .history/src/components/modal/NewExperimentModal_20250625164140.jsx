    ...
    try {
      const response = await fetch('/api/experiment', {
        ...
        body: JSON.stringify({
          ...
          session_id: uuidv4(),    // 👈 고유한 세션ID (uuid 라이브러리로 생성 가능)
          experiment_date: new Date().toISOString().slice(0, 10),  // 👈 "YYYY-MM-DD"
        }),
      });
  
      if (!response.ok) {
        const errorData = await response.text();
        console.error('실험 생성 실패 상세:', response.status, errorData);
