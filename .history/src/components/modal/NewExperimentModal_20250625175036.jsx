  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.message === 'Failed to fetch') {
        console.error('네트워크 에러 디버깅:');
        console.error('1. 백엔드 서버(localhost:8000)가 실행되고 있는지 확인');
        console.error('2. CORS 설정이 올바른지 확인');
        console.error('3. 프록시 설정(vite.config.js)이 올바른지 확인');
        console.error('4. 네트워크 연결 상태 확인');
        alert('서버에 연결할 수 없습니다. 백엔드 서버가 실행되고 있는지 확인해주세요.');
      } else {
